...
kable(habits, digits = 2,
      ...
      caption = "Table 1: Smoking Characteristics Broken Down by Smoking Group")
```

An important point to note here is that there are many other ways of defining dependent and non-dependent smokers other than the FTCD, and there are other key smoking measures such as the number of cigarettes smoked per day and the exhaled concentration of Carbon Monoxide (CO). These variables are included in the table, but we can take a closer look at their distribution to see how the groups differ.  
...
The first piece of information to present for attentional bias is the descriptive statistics for each condition broken down by smoking group. For each smoking group, the mean (and SD) RT to neutral stimuli, smoking stimuli, and the subsequent AB score is presented for each SOA condition. 
...
kable(AB.data, digits = 2,
      ...
      caption = "Table 3: Mean RT condition broken down by smoking group and SOA")
```
...
kable(AB.anova$ANOVA, digits = 3,
      caption = "Table 4: ANOVA results for main and interaction effects.")
```
...
kable(wisdm.p, digits = 2,
      col.names = c("Smoking group", "WISDM subscale", "Mean", "SD", "N"),
      caption = "Table 7: WISDM Primary Dependence Subscales")
```
...
These results can also be plotted to better visualise the similarities and differences between light and heavy smokers on primary and secondary dependence motives.
...
Looking back at this project, it is no surprise that it is uninformative, the sample size was just too small. Hopefully this information will be useful if you are performing a meta-analysis in the area or looking for studies to inform a power analysis. I have tried to include all the information you would need, if I have missed anything please let me know. I unfortunately cannot include the data as it was never included in the consent form that it would be posted publically. However, I have posted the .Rmd file on GitHub (link) for if you wanted to take a closer look at the code I have used. 
